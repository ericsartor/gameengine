...
export class Game {
	...
	constructor(options: { developmentMode?: boolean; gridSize?: number }) {
		// Set options
	...
	registerPawn(name: string, filePath: string) {
		if (this.started)
			throw new GameError(`tried to register pawn "${name}" after game started`);
	...
	registerActionHandler(context: string, action: string, handler: LogicFunction) {
		...
		if (this.actionHandlers[context][action])
			throw new GameError(
				`already registered action handler for action "${action}" in context "${context}"`
			);
	...
	async start() {
		...
		await Promise.all(
			this.pawnsToLoad.map(async ({ name, filePath }, i) => {
